...
https://{workspaceid}.z{xy}.blob.fabric.microsoft.com For data warehouse connection strings, use https://{GUID}-{GUID}.z{xy}.datawarehouse.fabric.microsoft.com that is, add z{xy} to the regular warehouse connection string found under SQL connection string. The GUIDs in the FQDN correspond to Tenant GUID in Base32 and Workspace GUID in Base32 respectively. This FQDN is not available as part of the DNS configurations for the private endp
